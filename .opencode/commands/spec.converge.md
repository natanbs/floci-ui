...
`/spec.implement` can complete it. This command MUST run only after
`/spec.implement` has run on the current `tasks.md`, and after `/spec.tasks` has produced a complete `tasks.md`.
...
prerequisite command to run (`/spec.specify` for a missing spec, `/spec.plan` for a missing plan,
`/spec.tasks` for missing tasks). Do not produce partial output.
...
- On `tasks_appended`: state how many tasks were appended under which phase, and recommend
  running `/spec.implement` to complete them; note that a follow-up converge
